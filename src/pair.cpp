...
double simil_correlation(colvec& col_i, colvec& col_j, bool nan) {
    double sdi = stddev(col_i, 1);
    double sdj = stddev(col_j, 1);
    if (sdi == 0.0 || sdj == 0.0)
        return nan ? std::numeric_limits<double>::quiet_NaN() : 0.0;
    double v1 = accu(col_i.t() * col_j);
    double v2 = mean(col_i) * mean(col_j) * col_i.n_rows;
    return ((v1 - v2) / col_i.n_rows) / (sdi * sdj);
}
        ...
        for (uword i = begin; i < end; i++) {
            ...
            for (uword j = 0; j < ncol; j++) {
                ...
                case 1:
                    ...
                    break;
                case 2:
                    simil = simil_correlation(col_i, col_j, nan);
                    break;
                case 3:
            ...
            for (std::size_t k = 0; k < simils.size(); k++) {
                if (simils[k] >= l || (nan && std::isnan(simils[k]))) {
