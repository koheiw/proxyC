...
**proxyC** v0.5 introduces masked similarity computation to improve execution time and memory usage. In the toy example, we compute similarity only between column vectors in `mt1` and `mt2` with the alphabetical names. We can achieve this by creating a pattern matrix `msk` using `mask()` and passing it to `simil()`. 
...
Masking has the greatest impact on execution time in other methods, because **proxyC** computes and save Dice coefficients only for unmasked pairs. We expect to see similar performance improvement in "jaccard", "ejaccard", "fjaccard", "edice", "hamann", "faith" and"simple matching".

```{r timeDice, fig.width=7, fig.height=4}
bm <- microbenchmark(
    unmasked = simil(mt1, mt2, margin = 2, method = "dice", mask = NULL, drop0 = TRUE),
    masked = simil(mt1, mt2, margin = 2, method = "dice", mask = msk, drop0 = TRUE)
)
...
Masking also dramatically reduces the the sizes of similarity matrices when `drop0 = TRUE`.
